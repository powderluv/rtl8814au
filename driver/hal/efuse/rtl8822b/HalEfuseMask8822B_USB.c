...
{
	return sizeof(Array_MP_8822B_MUSB) / sizeof(EF1Byte);
}

VOID EFUSE_GetMaskArray_MP_8822B_MUSB(u8* Array)
{
	EF2Byte len = EFUSE_GetArrayLen_MP_8822B_MUSB(), i = 0;

	for (i = 0; i < len; ++i)
		Array[i] = Array_MP_8822B_MUSB[i];
}

BOOLEAN EFUSE_IsAddressMasked_MP_8822B_MUSB(EF2Byte  Offset)
{
	int r = Offset / 16;
	int c = (Offset % 16) / 2;
	int result = 0;
